...
import { AnyObject, OmitFirstArg } from "@uwu-codes/utils";
...
interface Events {
	"error": (location: "main" | "cache" | "refresh", err: Error | string, extra?: unknown, threadId?: number) => void;
	...
	"download-start": (tags: Array<string>, folder: string, dir: string, threads: 1 | 2 | 3, usingAuth: boolean) => void;
...
class E621Downloader extends EventEmitter<Events> {
	...
	constructor(opts: Options) {
		...
		this.managers = {
			cache,
			refresh: undefined as unknown as RefreshManager // we have to do this weird due to the way RefreshManager works
		};
	...
	get refresh() {
		return this.managers.refresh;
	}


	get cacheObj() {
		return !this.current ? null : this.cache.get().data.find((v) => v.tags.join(" ").toLowerCase() === this.current.tags.join(" ").toLowerCase());
	}
	cached(id: number) {
	...
	private convertToCacheFormat(arr: Array<Post>) {
	...
	private get auth() {
		return this.options.auth === null ? null : ((this.options.auth as AnyObject<string>).basic  || Buffer.from(`${(this.options.auth as AnyObject<string>).username}:${(this.options.auth as AnyObject<string>).apiKey}`).toString("base64")) || null;
	}
	...
	count(arr: Array<Array<unknown>>, num: number): [start: number, end: number] {
		let a = 0, b = 0;
	...
	reset(folder?: string | null) {
		...
		this.current = {
			...
			total: 0,
			totalWithSkipped: 0,
			postsPerWorker: new Map<number, number>(),
			start: 0,
			end: 0,
			posts: [],
			tags: [],
	...
	private async endHandler(id: number) {
		...
		if (this.current.finishedCount >= this.current.threadCount) void this.complete();
	}
	...
	get sanitizeFolderName() {
		return E621Downloader.sanitizeFolderName.bind(E621Downloader);
	}

	private async fetchPosts(tags: Array<string>, auth: string | null, page: number, lastId?: number | null) {
		/* eslint-disable */
		const posts: Array<Post> = [];
		return new Promise<Array<Post>>((a, b) => {
			...
			https.request({
				...
				res
					...
					.on("end", async () => {
						...
						posts.push(...(d.posts as Array<any>).map<Post>((p: any) => ({
							id: p.id,
							url: p.file.url,
							ext: p.file.ext,
							md5: p.file.md5,
