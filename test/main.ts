...
process.nextTick(async () => {
	const v = await e
		...
		.on("skip", (threadId, id, reason, current, total) => console.log(`[Thread #${threadId}][${current}/${total}]: Skipped post #${id} due to ${reason === "cache" ? "it being cached" : "the file existing, and overwriteExisting being set to false."}`))
