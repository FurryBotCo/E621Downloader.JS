...
process.nextTick(async () => {
	const v = await e
		...
		.on("thread-done", (threadId, amount, time) => console.error(`[Thread #${threadId}]: Finished downloading ${amount} posts in ${ms(time, true)}`))
		...
		.on("download-done", (total, time) => console.error(`Finished downloading ${total} posts in ${ms(time, true)}`))
		.on("fetch-page", (page, count, time) => console.error(`Finished fetching page #${page} in ${ms(time, true)} (had ${count} posts)`))
		.on("fetch-finish", (total, time) => console.error(`Finished fetching ${total} posts in ${ms(time, true)}`))
